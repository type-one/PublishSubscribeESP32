...
namespace
{
    ...
    using traffic_light_state_v = std::variant<traffic_light_state::off, traffic_light_state::operable_initializing,
        traffic_light_state::operable_red, traffic_light_state::operable_orange, traffic_light_state::operable_green>;

    using traffic_light_event_v = std::variant<traffic_light_event::power_on, traffic_light_event::power_off,
        traffic_light_event::init_done, traffic_light_event::next_state>;
    ...
    private:
...
#else
        /**
         * @brief C++17 SFINAE trampoline: tries to call on_event(state, event); falls back to the long-overload error
         * path.
        ...
        /**
         * @brief Fallback selected when @c on_event(state, event) is ill-formed: logs an error and keeps the current
         * state.
         * @tparam State Concrete state type.
         * @tparam Event Concrete event type.
         */
    ...
    void traffic_light_fsm::handle_event(const traffic_light_event_v& event)
    {
        ...
        m_state = std::visit(tools::detail::overload { [&](const auto& state, const auto& evt)
                                 {
#if (__cplusplus >= 202002L) || (defined(_MSVC_LANG) && (_MSVC_LANG >= 202002L))
    ...
    {
        std::visit(tools::detail::overload { [&](const auto& state)
                       {
#if (__cplusplus >= 202002L) || (defined(_MSVC_LANG) && (_MSVC_LANG >= 202002L))
                           on_state(state);
#else
