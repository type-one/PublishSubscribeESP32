...
namespace
{
    ...
    /** @brief Demonstrates launching a periodic task pinned to core 0 and a worker task pinned to core 1 using explicit
     * CPU affinity. */
    void test_smp_tasks_cpu_affinity()
    {
        ...
        auto periodic_lambda = [&task1](const std::shared_ptr<smp_task_context>& context, const std::string& task_name)
        {
            ...
            task1.delegate(
                [](auto local_context, const auto& local_task_name)
                {
                    (void)local_context;
    ...
    /** @brief Demonstrates inter-core communication between a periodic and a worker task using a lock-free ring buffer
     * as a bidirectional pipe. */
    void test_smp_tasks_lock_free_ring_buffer()
    {
            ...
            = [&task1](const std::shared_ptr<smp_ring_task_context>& local_context, const std::string& task_name)
        {
            ...
            task1.delegate(
                [](const auto& delegate_context, const auto& delegate_task_name)
                {
    ...
    /** @brief Demonstrates chunked inter-core data transfer between a periodic and a worker task via a @c
     * tools::memory_pipe. */
    void test_smp_tasks_memory_pipe()
    {
...

        {
            ...
            task1.delegate(
                [&stop](const auto& delegate_context, const auto& task_name)
                {
                    std::printf("%s (core 1)\n", task_name.c_str());
            ...
            auto periodic_lambda
                = [&offset](const std::shared_ptr<smp_mem_task_context>& local_context, const std::string& task_name)
            {
                std::printf(" / %s (core 0)\n", task_name.c_str());
    ...
    /** @brief Exercises perfect-forwarding send/receive overloads of @c tools::memory_pipe with lvalue, rvalue, range,
     * conversion, and ISR variants. */
    void test_memory_pipe_perfect_forwarding()
    {
        ...
        auto converted = vector_convertible_data { std::vector<std::uint8_t>(
            conversion_payload.begin(), conversion_payload.end()) };
    ...
    /** @brief Demonstrates launching a high-priority periodic task alongside a low-priority worker task to illustrate
     * FreeRTOS priority scheduling. */
