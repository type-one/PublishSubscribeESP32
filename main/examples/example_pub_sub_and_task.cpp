...
namespace
{
    ...
    /** @brief Demonstrates the pub/sub pattern with synchronous and asynchronous observers, topic filtering,
     * subscriptions, unsubscriptions, and lambda handlers. */
    ...
    /** @brief Exercises perfect-forwarding subscribe/publish overloads of @c tools::sync_subject with lvalue, rvalue,
     * and implicit-conversion topic and payload arguments. */
     ...
     * implicit-conversion arguments, then drains all queued events. */
    ...
    /** @brief Demonstrates launching two generic tasks (one lambda, one function pointer) that run until a shared stop
     * flag is set. */
    ...
    /** @brief Exercises perfect-forwarding constructor overloads of @c tools::generic_task with lvalue callback, rvalue
     * callback, and implicit-conversion function pointer inputs. */
    ...
    /** @brief Demonstrates a periodic task firing at a fixed 20 ms interval and measures elapsed time between
    ...
    /** @brief Exercises perfect-forwarding constructor overloads of @c tools::periodic_task with lvalue startup
     * callback and rvalue periodic callback. */
    ...
    /** @brief Exercises perfect-forwarding @c add / @c add_range overloads of @c tools::histogram with scalar lvalue,
     * rvalue, implicit conversion, and range inputs. */
    ...
    /** @brief Demonstrates histogram statistics on @c fpm::fixed_16_16 samples without converting storage to
     * floating-point. */
    void test_histogram_fixed_16_16()
    {
        ...
        const std::array<fixed_scalar, 8U> samples = { fixed_scalar(1.5), fixed_scalar(0.5), fixed_scalar(1.5),
            fixed_scalar(-0.5), fixed_scalar(0.25), fixed_scalar(0.25), fixed_scalar(1.5), fixed_scalar(0.5) };
    ...
    public:
        ...
        /** @brief Prints histogram statistics: top value, average, median, variance, standard deviation, and Gaussian
         * metrics. */
        void display_stats()
        {
    ...
    /** @brief Demonstrates a periodic task publishing a sine-wave signal to both an async observer and a
     * histogram-collecting observer to illustrate integrated pub/sub with statistics. */
