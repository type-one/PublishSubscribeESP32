...
namespace
{
    ...
    /** @brief Demonstrates C++20 calendar utilities: computes the current year and years elapsed since the moon
     * landing. */
    ...
    void test_timer()
    {
...

        {
            ...
            timer_scheduler.add(
                "timer1", period_100ms, [&](tools::timer_handle) { val.store(test_value); },
            ...
            val.store(0);
            timer_scheduler.add(
                "timer2", period_100ms, [&](tools::timer_handle) { val.store(test_value); },
                tools::timer_type::one_shot);
            ...
            auto timer_id = timer_scheduler.add(
                "timer3", period_40ms, [&](tools::timer_handle) { ++count; }, tools::timer_type::periodic);
            ...
            timer_id = timer_scheduler.add(
                "timer4", period_40ms, [&](tools::timer_handle) { ++count; }, tools::timer_type::periodic);
