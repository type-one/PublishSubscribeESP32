...
namespace
{
    ...
    /** @brief Device manufacturing record with vendor, serial number, date, and tag, with bytepack serialization
     * support. */
    ...
    /** @brief Compound bytepack payload aggregating a sensor dictionary, manufacturing list, status flag, and value
     * vector. */
    ...
    void test_aggregated_bytepack_data()
    {
        ...
        aggr.m_dictionary
            = { { "sensor1", { sensor1_cpu_temperature_c, sensor1_gpu_temperature_c, sensor1_room_temperature_c } },
        ...
        if (message_type::aggregat == typ)
        {
            ...
            std::printf("%s %s %s %c %c \n", aggr_dup.m_list[1].manufacturing_date.c_str(),
                ...
                aggr_dup.m_list[1].tag[0], aggr_dup.m_list[1].tag[1]);
    ...
    /** @brief Exercises perfect-forwarding overloads of @c tools::data_task using lvalue, rvalue, and function pointer
     * callbacks. */
    void test_data_task_perfect_forwarding()
    {
...

        {
            auto task = std::make_unique<simple_data_task>(
                ...
                { std::printf("starting %s\n", task_name.c_str()); },
                [](const std::shared_ptr<data_task_context>&, const int& data, const std::string& task_name)
                { std::printf("%s received %d\n", task_name.c_str(), data); }, context, queue_depth,
                std::string("data_fwd_rvalue"), stack_size);
