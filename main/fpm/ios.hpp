...
namespace fpm
{
    ...
    std::basic_ostream<CharT>& operator<<(std::basic_ostream<CharT>& os, fixed<B, I, F, R> x) noexcept
    {
        ...
        using buffer_t
            = std::array<CharT, worst_case_constant_size + worst_case_digit_count * 2 + worst_case_suffix_size>;
        buffer_t buffer;
    ...
    std::basic_istream<CharT, Traits>& operator>>(std::basic_istream<CharT, Traits>& is, fixed<B, I, F, R>& x)
    {
        ...
        const auto& is_valid_character = [](char ch)
        {
            ...
            // std::num_get), but otherwise we cannot parse hexfloats and "infinity". This is a known issue with the
            // spec (LWG #2381).
