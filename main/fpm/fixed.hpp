...
namespace fpm
{
    ...
    //! unsigned type. \tparam IntermediateType the integer type used to store intermediate results during calculations.
    ...
    public:
        ...
        static constexpr fixed from_fixed_point(T value) noexcept
        {
            ...
            return fixed(
                static_cast<BaseType>(value / (T(1) << (NumFractionBits - FractionBits))), raw_construct_tag {});
        }
        ...
        {
            return fixed(
                static_cast<BaseType>(value * (T(1) << (FractionBits - NumFractionBits))), raw_construct_tag {});
        }
        ...
        template <unsigned long long NumFraction, typename TInt, typename TFraction,
            typename std::enable_if<(NumFraction > FRACTION_MULT) && std::is_integral<TInt>::value
                && std::is_integral<TFraction>::value>::type* = nullptr>
        static constexpr fixed from_custom_fraction(TInt integer_value, TFraction fraction_value) noexcept
        {
            ...
            const IntermediateType frac_part = static_cast<IntermediateType>(fraction_value) * FRACTION_MULT
                / static_cast<IntermediateType>(NumFraction);
            const IntermediateType two_frac_part = static_cast<IntermediateType>(fraction_value) * FRACTION_MULT * 2
                / static_cast<IntermediateType>(NumFraction);
            ...
            if constexpr (EnableRounding)
            {
                return fixed(static_cast<BaseType>(int_part + frac_part + (two_frac_part % 2)), raw_construct_tag {});
            }
        ...
        fixed& operator*=(const fixed& rhs_value) noexcept
        {
            if constexpr (EnableRounding)
            {
                ...
                // To correctly round the last bit in the result, we need one more bit of information.
                // We do this by multiplying by two before dividing and adding the LSB to the real result.
                const auto value = (static_cast<IntermediateType>(m_value) * rhs_value.m_value) / (FRACTION_MULT / 2);
     ...
     */
    template <typename B, typename I, unsigned int F, bool R>
    constexpr fixed<B, I, F, R> operator+(
        const fixed<B, I, F, R>& lhs_value, const fixed<B, I, F, R>& rhs_value) noexcept
    {
    ...
    namespace detail
    {
        ...
        static constexpr int max_digits10(int bit_count)
        {
            ...
            return static_cast<int>((decimal_digits_temp_type { bit_count } * k_log10_2_scaled_8_24
        ...
        static constexpr int digits10(int bit_count)
        {
            ...
            return static_cast<int>(
                (decimal_digits_temp_type { bit_count } * k_log10_2_scaled_8_24) >> k_scale_shift_bits);
        }
...
namespace std
{
    ...
    struct hash<fpm::fixed<B, I, F, R>>
    {
        ...
        result_type operator()(argument_type arg) const
            noexcept(noexcept(std::declval<std::hash<B>>()(arg.raw_value())))
        {
