 ...
 * @brief Imported-style unit tests for portable_concurrency v2 promise_result.
 */
...
/**
 * @brief Verifies destroying an unsatisfied promise_result yields broken_promise.
 */
...
/**
 * @brief Verifies void promise_result can satisfy its future without payload.
 */
...
TEST(PromiseResultTest, repeated_completion_without_future_is_harmless)
{
    ...
    EXPECT_NO_FATAL_FAILURE(promise.set_error(result_error::execution_failure));
}
...
{
    promise_result<int> original;
    promise_result<int> moved = std::move(original);
...
{
    promise_result<int> promise;
    promise_result<int> moved = std::move(promise);
...
{
    EXPECT_FALSE((std::is_constructible_v<promise_result<int>, std::allocator_arg_t, std::allocator<int>>));
}
...
TEST(PromiseResultTest, canceler_is_not_called_by_constructor)
{
    ...
    promise_result<int> promise(canceler_arg, [&]() { ++call_count; });
...
TEST(PromiseResultTest, canceler_is_called_once_if_future_destroyed_before_set)
{
    size_t call_count = 0U;
    {
        auto pair = make_result_promise<int>(canceler_arg, [&]() { ++call_count; });
        auto promise = std::move(pair.first);
        auto future = std::move(pair.second);
...
TEST(PromiseResultTest, canceler_supports_move_only_cleanup)
{
    ...
    auto pair = make_result_promise<int>(canceler_arg,
