...
namespace
{
    ...
    {
        static const std::vector<float> coefficients = { -1.50F, -1.25F, -1.00F, -0.75F, -0.50F, -0.25F, 0.00F, 0.25F,
            0.50F, 0.75F, 1.00F, 1.25F, 1.50F, 1.10F, 0.90F, 0.70F, 0.30F, 0.10F, -0.10F, -0.30F, -0.70F, -0.90F,
            -1.10F, -1.30F, 1.40F, 1.20F, 0.80F, 0.60F, 0.40F, 0.20F, -0.20F, -0.40F, -0.60F, -0.80F, -1.20F, -1.40F,
            1.35F, 1.05F, 0.55F, 0.15F, -0.15F, -0.55F, -1.05F, -1.35F, 1.45F, 0.95F, 0.45F, -0.05F, -0.45F, -0.95F,
            -1.45F, 1.30F, 0.85F, 0.35F, -0.35F, -0.85F, -1.30F, 1.15F, 0.65F, 0.05F, -0.65F, -1.15F, 1.50F, -1.50F };
        return coefficients;
    }
    ...
    pco::future_result<matrix_4x4<float>> coroutine_matrix_multiply_float(worker_result_task& worker,
        const std::shared_ptr<worker_result_context>& context, matrix_4x4<float> left, matrix_4x4<float> right)
    {
    ...
    pco::future_result<matrix_4x4<fpm::fixed_16_16>> coroutine_matrix_multiply_fixed(worker_result_task& worker,
        const std::shared_ptr<worker_result_context>& context, matrix_4x4<fpm::fixed_16_16> left,
    ...
    pco::future_result<int> coroutine_await_future_job(worker_result_task& worker,
        const std::shared_ptr<worker_result_context>& context, pco::future_result<int> upstream)
    {
        co_await worker.schedule();
    ...
    pco::future_result<int> coroutine_process_sample_on_worker(worker_result_task& worker,
        const std::shared_ptr<worker_result_context>& context,
        const std::shared_ptr<stress_worker_processor>& processor, int input_value)
    {
        co_await worker.schedule();
        context->loop_counter.fetch_add(1);
    ...
    pco::future_result<int> coroutine_alternating_chain_on_two_workers(worker_result_task& worker_first,
        worker_result_task& worker_second, const std::shared_ptr<worker_result_context>& context_first,
        const std::shared_ptr<worker_result_context>& context_second,
        const std::shared_ptr<stress_worker_processor>& processor, int input_value, int stage_count)
    {
...
TEST(PortableConcurrencyWorkerTaskResultTest, PromiseResultManualFulfillment)
{
    ...
    auto chained = std::move(future).then_value([](int value) { return value * 2; });
...
TEST(PortableConcurrencyWorkerTaskResultTest, PeriodicTaskFeedsWorkerStressWithoutCoroutines)
{
    ...
    auto periodic = [worker_ptr, worker_context, processor](
                        const std::shared_ptr<periodic_stress_context>& local_context, const std::string&)
    {
        ...
        auto chained = worker_ptr
                           ->delegate_async(
                               [processor](const std::shared_ptr<worker_result_context>& worker_ctx, const std::string&,
                                   int input_value)
                               {
    ...
    {
        periodic_stress_task producer(std::move(startup), std::move(periodic), stress_context,
            "periodic_stress_producer", periodic_interval, periodic_stack_size);
...
TEST(PortableConcurrencyWorkerTaskResultTest, AlternatingChainAcrossTwoWorkersWithoutCoroutines)
{
    ...
    for (int stage_index = 0; stage_index < stage_count; ++stage_index)
    {
        ...
        auto stage_future = active_worker->delegate_async(
            [processor](const std::shared_ptr<worker_result_context>& local_context, const std::string&,
                int input_chain_value, int current_stage)
            {
...
TEST(PortableConcurrencyWorkerTaskResultTest, MatrixPairwiseReductionAcrossWorkersWithoutCoroutines)
{
    ...
    while (reduce_float.size() > 1U)
    {
        ...
        for (std::size_t pair_index = 0U; (pair_index + 1U) < reduce_float.size(); pair_index += 2U)
        {
            ...
            jobs.emplace_back(worker_ptr->delegate_async(
                [](const std::shared_ptr<worker_result_context>& context, const std::string&, matrix_4x4<float> left,
                    matrix_4x4<float> right)
                {
                    context->loop_counter.fetch_add(1);
                    return multiply_matrix_4x4(left, right);
                },
                reduce_float.at(pair_index), reduce_float.at(pair_index + 1U)));
        }
    ...
    while (reduce_fixed.size() > 1U)
    {
        ...
        for (std::size_t pair_index = 0U; (pair_index + 1U) < reduce_fixed.size(); pair_index += 2U)
        {
            ...
            jobs.emplace_back(worker_ptr->delegate_async(
                [](const std::shared_ptr<worker_result_context>& context, const std::string&,
                    matrix_4x4<fpm::fixed_16_16> left, matrix_4x4<fpm::fixed_16_16> right)
                {
                    context->loop_counter.fetch_add(1);
                    return multiply_matrix_4x4(left, right);
                },
                reduce_fixed.at(pair_index), reduce_fixed.at(pair_index + 1U)));
        }
...
TEST(PortableConcurrencyWorkerTaskResultTest, CoroutineScheduleFlow)
{
    ...
    auto scheduled = coroutine_schedule_job(*worker, context, 5).then_value([](int value) { return value + 4; });
...
TEST(PortableConcurrencyWorkerTaskResultTest, PeriodicTaskFeedsWorkerStressWithCoroutines)
{
    ...
    {
        periodic_stress_task producer(std::move(startup), std::move(periodic), stress_context,
            "periodic_coro_stress_producer", periodic_interval, periodic_stack_size);
...
TEST(PortableConcurrencyWorkerTaskResultTest, MatrixPairwiseReductionAcrossWorkersWithCoroutines)
{
    ...
    while (reduce_float.size() > 1U)
    {
        ...
        {
            const std::size_t worker_index = (pair_index / 2U) % matrix_worker_count;
            jobs.emplace_back(coroutine_matrix_multiply_float(*workers.at(worker_index), contexts.at(worker_index),
                reduce_float.at(pair_index), reduce_float.at(pair_index + 1U)));
        }
    ...
    while (reduce_fixed.size() > 1U)
    {
        ...
        {
            const std::size_t worker_index = (pair_index / 2U) % matrix_worker_count;
            jobs.emplace_back(coroutine_matrix_multiply_fixed(*workers.at(worker_index), contexts.at(worker_index),
                reduce_fixed.at(pair_index), reduce_fixed.at(pair_index + 1U)));
        }
