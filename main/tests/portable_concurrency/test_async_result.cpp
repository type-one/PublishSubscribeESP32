...
namespace
{
    ...
    {
        auto future = pco::async_result(pco::inplace_executor, [] { return 42; });
    ...
    {
        auto future
            = pco::async_result(pco::inplace_executor, [](int left, int right) { return left + right; }, 20, 22);
    ...
    {
        bool executed = false;
        auto future = pco::async_result(pco::inplace_executor, [&executed] { executed = true; });
    ...
    {
        auto future = pco::async_result(pco::inplace_executor, [] { return 21; });
    ...
    TEST(AsyncResultTest, executes_callable_on_specified_executor_thread)
    {
        ...
        auto future = pco::async_result(exec, [] { return std::this_thread::get_id(); });
    ...
    TEST(AsyncResultTest, unwraps_nested_future_result)
    {
        ...
        auto future = pco::async_result(exec, [exec] { return pco::async_result(exec, [] { return 100500; }); });

        // result type must be pco::future_result<int>, not pco::future_result<pco::future_result<int>>
        static_assert(std::is_same<decltype(future), pco::future_result<int>>::value,
    ...
    TEST(AsyncResultTest, unwraps_nested_shared_result)
    {
        ...
        auto future = pco::async_result(exec, [exec] { return pco::async_result(exec, [] { return 42; }).share(); });
    ...
    TEST(AsyncResultTest, destroys_callable_after_invocation)
    {
        ...
        auto future = pco::async_result(exec, [sp = std::exchange(sp, nullptr)] { return *sp; });
