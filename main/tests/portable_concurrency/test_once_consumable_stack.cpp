...
::testing::AssertionResult monotonic_sequence(const char *, const char *,
                                              It first, It last) {
  const auto it = std::adjacent_find(
      first, last, [](record lhs, record rhs) { return rhs.id - lhs.id != 1; });
  if (it == last)
    return ::testing::AssertionSuccess();

  if (it->id == (it + 1)->id)
    return ::testing::AssertionFailure()
           << "Dublicated record {task_id: " << it->task_id
           << ", id: " << it->id << "}";

  return ::testing::AssertionFailure()
         << "Task " << it->task_id << " has missing records between " << it->id
         << " and " << (it + 1)->id;
}
...
/**
 * \brief Verifies concurrent push until consume for once consumable queue.
 */
TEST(OnceConsumableQueueTests, concurrent_push_until_consume) {
  ...
  for (auto &f : futures) {
    ...
    auto ids_range = std::equal_range(
        records.begin(), records.end(), last,
