...
namespace
{
    ...
    {
        pco::packaged_task_result<int(int)> task([](int v) { return v + 1; });
        EXPECT_TRUE(task.valid());
    }

    TEST(PackagedTaskResultTest, invoke_with_value_argument_delivers_result)
    {
        pco::packaged_task_result<int(int)> task([](int v) { return v * 2; });
    ...
    {
        pco::packaged_task_result<int(std::unique_ptr<int>)> task([](std::unique_ptr<int> p) { return *p; });
    ...
    {
        pco::packaged_task_result<pco::future_result<int>()> task([] { return pco::make_ready_result(42); });
    ...
    {
        pco::packaged_task_result<pco::shared_result<int>()> task([] { return pco::make_ready_result(7).share(); });
    ...
    TEST(PackagedTaskResultTest, unsatisfied_task_destruction_yields_broken_promise)
    {
        ...
        {
            pco::packaged_task_result<int()> task([] { return 5; });
    ...
    {
        pco::packaged_task_result<std::size_t(std::string)> task([](const std::string& s) { return s.size(); });
    ...
    TEST(PackagedTaskResultTest, destroys_callable_after_invocation)
    {
        ...
        pco::packaged_task_result<int(int)> task([sp = std::exchange(sp, nullptr)](int val) { return val + *sp; });
