...
namespace
{
    ...
    TEST(NextResultTest, future_next_runs_on_success)
    {
        ...
        auto chained = std::move(source).next([](int value) { return value + 5; });
    ...
    TEST(NextResultTest, future_next_skips_on_error)
    {
        ...
        auto chained = std::move(source).next(
            [&called](int)
            {
                called = true;
                return 0;
    ...
    TEST(NextResultTest, future_next_executor_dispatches)
    {
        ...
        auto chained = std::move(source).next(exec, [](int value) { return value * 3; });
    ...
    TEST(NextResultTest, shared_next_runs_on_success)
    {
        ...
        auto chained = shared.next([](const int& value) { return value + 4; });
    ...
    TEST(NextResultTest, shared_next_executor_dispatches)
    {
        ...
        auto chained = shared.next(pco::inplace_executor, [](const int& value) { return value * 2; });
    ...
    TEST(NextResultTest, then_value_executor_registration_is_non_blocking)
    {
        ...
        auto registration = std::async(std::launch::async, [&source, &exec]() mutable
            { return std::move(source).then_value(exec, [](int value) { return value + 1; }); });
    ...
    TEST(NextResultTest, then_error_executor_registration_is_non_blocking)
    {
        ...
        auto registration = std::async(std::launch::async, [&source, &exec]() mutable
            { return std::move(source).then_error(exec, [](pco::result_error /*err*/) { return -1; }); });
    ...
    TEST(NextResultTest, then_result_executor_registration_is_non_blocking)
    {
        ...
        auto registration = std::async(std::launch::async,
            [&source, &exec, &fn_called]() mutable
            {
                return std::move(source).then_result(exec,
                    [&fn_called](auto r)
                    {
                        fn_called = true;
                        return r;
