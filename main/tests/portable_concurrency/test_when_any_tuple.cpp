...
/**
 * \brief Verifies empty sequence for when any tuple.
 */
TEST(WhenAnyTupleTest, empty_sequence) {
  auto res_fut = pco::when_any();
...
/**
 * \brief Verifies single future for when any tuple.
 */
TEST(WhenAnyTupleTest, single_future) {
  auto p = pco::make_promise<int>();
  auto f = pco::when_any(std::move(p.second));
  ASSERT_TRUE(f.valid());
...
/**
 * \brief Verifies single shared future for when any tuple.
 */
TEST(WhenAnyTupleTest, single_shared_future) {
  auto p = pco::make_promise<int>();
  auto raw_f = p.second.share();
  auto f = pco::when_any(raw_f);
  ASSERT_TRUE(f.valid());
...
/**
 * \brief Verifies single ready future for when any tuple.
 */
TEST(WhenAnyTupleTest, single_ready_future) {
  auto raw_f = pco::make_ready_future(123);
  auto f = pco::when_any(std::move(raw_f));
  ASSERT_TRUE(f.valid());
...
/**
 * \brief Verifies single ready shared future for when any tuple.
 */
...
/**
 * \brief Verifies single error future for when any tuple.
 */
...
/**
 * \brief Verifies single error shared future for when any tuple.
 */
...
/**
 * \brief Verifies next ready futures dont affect result before get for when any tuple.
 */
TEST(WhenAnyTupleTest, next_ready_futures_dont_affect_result_before_get) {
  pco::promise<int> p0;
  pco::promise<std::string> p1;
  pco::promise<void> p2;

  auto f = pco::when_any(p0.get_future(), p1.get_future(), p2.get_future());
  ASSERT_TRUE(f.valid());
  EXPECT_FALSE(f.is_ready());
...
/**
 * \brief Verifies next ready futures dont affect result after get for when any tuple.
 */
...
/**
 * \brief Verifies concurrent result delivery for when any tuple.
 */
TEST(WhenAnyTupleTest, concurrent_result_delivery) {
  ...
  auto f = pco::when_any(t0.get_future(), p1.get_future(), t2.get_future());
  ASSERT_TRUE(f.valid());
  EXPECT_FALSE(f.is_ready());
...
/**
 * \brief Verifies multiple futures for when any tuple.
 */
TEST_P(WhenAnyTupleTest, multiple_futures) {
  ...
  pco::promise<std::unique_ptr<int>> p2;
  auto raw_f2 = p2.get_future();
  pco::promise<void> p3;
  auto raw_f3 = p3.get_future().share();
  pco::promise<future_tests_env &> p4;
  auto raw_f4 = p4.get_future();

  auto f = pco::when_any(std::move(raw_f0), raw_f1, std::move(raw_f2), raw_f3,
                        std::move(raw_f4));
  ASSERT_TRUE(f.valid());
...
/**
 * \brief Verifies multiple futures one initially ready for when any tuple.
 */
TEST_P(WhenAnyTupleTest, multiple_futures_one_initially_ready) {
  ...
  pco::shared_future<int> raw_f0 = p0.get_future();
  pco::future<std::string> raw_f1 = p1.get_future();
  pco::shared_future<std::unique_ptr<int>> raw_f2 = p2.get_future();
  pco::future<void> raw_f3 = p3.get_future();
  pco::shared_future<future_tests_env &> raw_f4 = p4.get_future();
  ...
  auto f = pco::when_any(raw_f0, std::move(raw_f1), raw_f2, std::move(raw_f3),
                        raw_f4);
  ASSERT_TRUE(f.valid());
